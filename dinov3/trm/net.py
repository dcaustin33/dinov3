...
def rms_norm(
    hidden_states: torch.Tensor, variance_epsilon: float = 1e-5
) -> torch.Tensor:
...
class TRM(torch.nn.Module):
    def __init__(
        ...
        self.latent_y_embedding = nn.Parameter(
            trunc_normal_init_(torch.empty((1, self.latent_y_dim)), std=embed_init_std)
        )
        self.latent_z_embedding = nn.Parameter(
            trunc_normal_init_(torch.empty((1, self.latent_z_dim)), std=embed_init_std)
        )
    ...
    def latent_recursion(
        self, x: torch.Tensor, y_latent: torch.Tensor, z_latent: torch.Tensor
    ):
        x_dim = x.shape[-1]
    ...
    def _initialize_latents(self, batch_size: int):
        """Initialize latent embeddings for a batch."""
    ...
    def _compute_losses(
        ...
        q_loss = F.binary_cross_entropy_with_logits(
            q_logits.squeeze(-1), (y_hat == y_true).float()
        )
        return cls_loss + q_loss
    ...
    def _filter_samples(
        ...
        """Filter samples based on confidence threshold and update stopping layers."""
        if step >= self.n_supervision - 1:
            # Last layer - mark remaining samples
            if not stopped_mask[original_indices].all():
                stopping_layers[original_indices[~stopped_mask[original_indices]]] = (
                    step + 1
                )
        ...
        if continue_mask.any():
            return (
                x[continue_mask],
                y_true[continue_mask],
                y_latent[continue_mask],
                z_latent[continue_mask],
                original_indices[continue_mask],
                True,
            )
        else:
            return None, None, None, None, None, False  # All samples stopped
    ...
    def deep_recursion(
        ...
        for _ in range(self.t_recursion_steps - 1):
            ...
            y_latent_new, z_latent_new = self.latent_recursion(
                x, y_latent.detach() + y_embedding, z_latent.detach() + z_embedding
            )
            self.net.requires_grad_(True)
            y_latent = y_latent_new
            z_latent = z_latent_new
        y_latent, z_latent = self.latent_recursion(
            x, y_latent + y_embedding, z_latent + z_embedding
        )
    ...
    def forward(
        ...
        y_latent, z_latent = self._initialize_latents(batch_size)
        ...
        for step in range(self.n_supervision):
            samples_per_layer.append(x.shape[0])

            y_latent, z_latent, cls_logits, q_logits = self.deep_recursion(
                x,
                y_latent.clone(),
                z_latent.clone(),
                self.latent_y_embedding,
            ...
            x, y_true, y_latent, z_latent, original_indices, should_continue = (
    ...
    def forward_eval(self, x: torch.Tensor, y_true: torch.Tensor) -> dict:
        ...
        for step in range(self.n_supervision):
            ...
            layer_accuracies.append((y_hat == y_true).float().mean().item())
